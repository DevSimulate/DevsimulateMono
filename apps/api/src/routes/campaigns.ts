 ...
 */
/**
 * POST /campaigns/:id/invites/deadline-extended
 * Body: { dryRun?: boolean }
 *
 * Tells candidates who are MID-ASSESSMENT that the round has been extended.
 *
 * Narrower than closing-soon by design: only people who have actually started
 * and not finished. Someone who never opened the link has no deadline pressure
 * to relieve, so for them this would just be an extra nudge — they get the
 * final call on the deadline morning instead, and the quota is spent once
 * rather than twice.
 *
 * Requires a deadline to be set; "we've extended it to nothing in particular"
 * is not a message worth sending.
 */
router.post("/:id/invites/deadline-extended", async (req: Request, res: Response): Promise<void> => {
  const { userId } = (req as AuthenticatedRequest).user;
