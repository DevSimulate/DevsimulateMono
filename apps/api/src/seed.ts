...
- Customer dashboard data is denormalized into a reporting cache that was rebuilt during the SQL migration`,
  ...
  const tickets = [
    ...
    },
    {
      ...
      description: "Finance has flagged 11 invoices this quarter with duplicate numbers — two separate invoices both assigned INV-2026-00034, for example. They only discovered it when a customer called about a payment that was applied to the wrong account. The invoice sequence comes from a database stored procedure (dbo.usp_GetNextInvoiceSequence) that the repository calls, but somehow two invoices are getting the same number. It never happens during normal usage but appears under traffic spikes.",
      difficulty: Difficulty.MID,
      filesInvolved: ["src/NovaTechCRM.Repositories/InvoiceRepository.cs", "src/NovaTechCRM.Infrastructure/Database/StoredProcedures/usp_GetNextInvoiceSequence.sql", "src/NovaTechCRM.Services/InvoiceService.cs"],
    ...
    {
      // TWO-LAYER TICKET.
      // LAYER 1 (surface, obvious): the intern's Parallel.ForEach shares one EF
      //   DbContext (not thread-safe) → concurrency exception + corruption. Everyone
      //   fixes this. Hidden test verifies it.
      // LAYER 2 (hidden, anticipation): the ticket says the job "times out" and Ops
      //   "re-runs it" — but the dedup check (alreadyBilled, keyed on an OrderId these
      //   monthly invoices never set) is broken, so a re-run DOUBLE-BILLS customers.
      //   The ticket never mentions double-billing — a senior INFERS the idempotency
      //   risk from "times out + gets re-run". A "fix this" conduit fixes Layer 1 and
      //   ships. Hidden test re-runs billing and asserts no double-billing.
      id: "ticket-nova-130-seed-id-trap",
      title: "NOVA-130: Monthly Billing Run Times Out",
      description: "Our monthly billing job loops over every active customer and runs a few queries each to assemble their invoice. It's gotten slow and now times out before finishing — when it hangs, Ops just kills the process and re-runs the whole job. An intern already opened a draft PR that speeds it up by wrapping the per-customer work in Parallel.ForEach so customers are processed concurrently — it runs much faster on his laptop. But since it went to staging we're seeing intermittent 'A second operation was started on this context instance before a previous operation completed' errors, and a handful of customers got invoices with the wrong line items. Make the billing run finish in time WITHOUT errors or data corruption, and make sure it's safe to run the way Ops actually runs it. Review the intern's approach before building on it.",
      difficulty: Difficulty.SENIOR,
      ...
      rubric: { diagnosis: "TWO LAYERS. LAYER 1 (most candidates get this): the intern's Parallel.ForEach shares a single injected NovaTechDbContext — EF Core's DbContext is NOT thread-safe — which causes BOTH the 'second operation started on this context' exception AND the wrong-line-item corruption. PENALIZE 'parallelise more / add a lock / raise the timeout / retry' (confident-but-wrong). LAYER 2 — THE REAL SENIOR SIGNAL (anticipation): the ticket says the job times out and Ops re-runs it. Did they ANTICIPATE that re-running double-bills, and trace it to the broken dedup — `alreadyBilled` is keyed on `Invoice.OrderId`, but these monthly invoices span multiple orders and never set OrderId, so the HashSet is always empty and the second run re-creates every invoice? Full marks ONLY if they caught the idempotency risk WITHOUT being told about double-billing. A fix that nails Layer 1 but ignores Layer 2 is a mid-level answer, not senior.", design: "Layer 1: set-based single query (best), or context-per-task via IDbContextFactory + throttle. Layer 2: a CORRECT idempotency fix — dedup by customer+period (or track billed orders properly) so a re-run creates nothing new; note that the deterministic InvoiceNumber + a unique index is a backstop, not a real fix (it would throw on the second run rather than skip cleanly). Reward 'I chose X over Y because Z, at the cost of W' on both layers.", communication: "VERIFICATION: how do they KNOW it's correct — both that the corruption is gone (run concurrently, assert line items match a single-threaded baseline) AND that it's idempotent (run it twice, assert no duplicate invoices)? Reward a verification plan that covers the re-run case, not just the concurrency case.", execution: "Removes the concurrency exception + corruption (Layer 1) AND is idempotent under re-run (Layer 2). A fix that only addresses concurrency and double-bills on re-run does NOT fully pass." },
      expectedMinutes: 120,
    },
    {
