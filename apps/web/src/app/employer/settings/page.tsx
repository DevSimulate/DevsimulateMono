  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ color: "#e5e7eb" }}>
      ...
      <main className="flex-1 px-8 py-6 max-w-2xl space-y-5">
        ...
        {/* Branding */}
        <div className="rounded-xl p-6" style={{ background: "#111", border: "1px solid #222" }}>
          <div className="text-sm font-bold text-white mb-1 flex items-center gap-2"><Palette size={15} /> Branding</div>
          <p className="text-xs mb-4" style={{ color: "#555" }}>
            Applied to your candidate apply page, leaderboard, and contest links.
          </p>

          <label className="block text-xs font-semibold mb-1.5" style={{ color: "#888" }}>Brand name (shown to candidates)</label>
          <input value={brandName} onChange={(e) => setBrandName(e.target.value)} disabled={!isAdmin}
            placeholder={data?.orgName ?? "LMKR"}
            className="w-full rounded-lg px-3 py-2.5 text-sm outline-none mb-4 disabled:opacity-60" style={inputStyle} />

          <label className="block text-xs font-semibold mb-1.5" style={{ color: "#888" }}>Logo URL</label>
          ...
          {logoUrl && (
            <div className="mb-4 flex items-center gap-3">
              ...
              <span className="text-xs" style={{ color: "#555" }}>Preview</span>
            </div>
          )}

          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: "#888" }}>Primary colour</label>
              <div className="flex items-center gap-2">
                <input type="color" value={previewColor}
                  ...
                  style={{ background: "none", border: "1px solid #2a2a2a", padding: "2px" }} />
            ...
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: "#888" }}>Accent colour</label>
              <div className="flex items-center gap-2">
          ...
          {/* Live preview strip */}
          <div className="rounded-lg p-4 mb-4" style={{ background: "#0d0d0d", border: "1px solid #1e1e1e" }}>
            <div className="text-xs uppercase tracking-widest mb-2" style={{ color: "#555" }}>Preview</div>
            <div className="flex items-center gap-3">
              {logoUrl ? (
                <img src={logoUrl} alt="logo" className="h-8 rounded object-contain" style={{ background: "#1a1a1a", padding: "2px" }} />
              ) : (
              ...
              <div>
                <div className="text-sm font-black text-white">{brandName || data?.orgName || "Your Company"}</div>
                <div className="text-xs" style={{ color: "#888" }}>is hiring</div>
              </div>
              <div className="ml-auto">
                <div className="px-3 py-1.5 rounded-lg text-xs font-bold text-white" style={{ background: previewColor }}>
                  Join Campaign
                </div>
              </div>
            </div>
          ...
          {isAdmin && (
            <button onClick={save} disabled={saving} className="px-5 py-2.5 rounded-lg text-sm font-bold text-white flex items-center gap-2" style={{ background: previewColor }}>
