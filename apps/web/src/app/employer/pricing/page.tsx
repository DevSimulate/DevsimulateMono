      ...
      <main className="flex-1 px-8 py-10">
        ...
        {error && (
          <div className="max-w-md mx-auto mb-6 rounded-lg px-4 py-3 text-sm text-center"
            style={{ background: "#1c0000", border: "1px solid #7f1d1d", color: "#b42318" }}>
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-5 max-w-4xl mx-auto">
          {PLANS.map((p) => (
            <div key={p.id} className="rounded-2xl p-6 flex flex-col relative"
              style={{
                background: p.highlight ? "#111827" : "#ffffff",
                border: `1px solid ${p.highlight ? "#4338ca" : "#e4e7ec"}`,
              }}>
              {p.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-xs font-bold px-3 py-1 rounded-full text-[#131722]"
                  style={{ background: "#4338ca" }}>Most Popular</div>
              )}
              ...
              <div className="space-y-2.5 mb-6 flex-1">
                {p.features.map((f) => (
                  <div key={f} className="flex items-start gap-2 text-xs" style={{ color: "#5a6472" }}>
                    <Check size={14} style={{ color: "#067647" }} className="mt-0.5 shrink-0" />
                    <span>{f}</span>
                  </div>
                ))}
              </div>
              ...
              <button onClick={() => choose(p)} disabled={busy === p.id}
                ...
                style={{
                  background: p.highlight ? "linear-gradient(135deg,#4338ca,#3f37c9)" : "#eef1f5",
                  color: p.highlight ? "#fff" : "#131722",
                  border: p.highlight ? "none" : "1px solid #d5d9e0",
                }}>
        ...
        <div className="text-center mt-10">
          <p className="text-xs" style={{ color: "#8a93a3" }}>
            ...
            <a href="mailto:ossama@devsimulate.com" style={{ color: "#4338ca" }}>Talk to us about volume pricing</a>
          </p>
        </div>
      </main>
    </div>
